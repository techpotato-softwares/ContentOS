...
def assert_byok_allowed(tenant: Any) -> None:
    plan = get_plan(getattr(tenant, "plan_tier", None) or "starter")
...
def ai_settings_public(tenant: Any, *, keys: dict[str, bool] | None = None) -> dict[str, Any]:
    """Safe GET/PUT response shape — never includes raw API keys."""
    ...
    limit = int(getattr(tenant, "ai_posts_quota_monthly", None) or plan["quota"] or 0)
    used = 0 if stored_month != month else int(getattr(tenant, "ai_posts_used_month", None) or 0)
    key_flags = keys or peek_tenant_ai_key_flags(
        int(tenant.tenant_id),
        getattr(tenant, "ai_secret_arn", None),
    )
    return {
        "aiBillingMode": (getattr(tenant, "ai_billing_mode", None) or "platform"),
        "planTier": plan["id"],
        ...
        "keys": {
            "openaiConfigured": bool(key_flags.get("openaiConfigured")),
            "geminiConfigured": bool(key_flags.get("geminiConfigured")),
        },
