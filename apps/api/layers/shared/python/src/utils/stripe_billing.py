...
def _as_dict(obj: Any) -> dict[str, Any]:
    """Normalize Stripe SDK objects / mappings to a plain dict."""
    if obj is None:
        return {}
...
def create_checkout_session(
    ...
    *,
    ...
    require_stripe_configured()
    assert_can_subscribe(tenant, "stripe")
...
def process_stripe_event(session: Session, event: dict[str, Any]) -> dict[str, Any]:
    ...
    session.add(
        StripeWebhookEvent(event_id=event_id, event_type=event_type, processed_at=_utcnow())
    )
    ...
    if event_type == "checkout.session.completed":
        ...
        if sub_id:
            ...
            tenant = sync_tenant_from_subscription(
                session, subscription=_as_dict(subscription), tenant_id=tenant_id
            )
        ...
        if event_type == "customer.subscription.deleted" and tenant:
            ...
            if (tenant.billing_gateway or "").lower() == "stripe":
        ...
        if tenant:
            ...
            if sub_id:
                stripe = get_stripe()
                subscription = stripe.Subscription.retrieve(sub_id)
                sync_tenant_from_subscription(session, subscription=_as_dict(subscription))
            else:
                tenant.billing_status = "active"
                tenant.updated_at = _utcnow()
                session.add(tenant)
