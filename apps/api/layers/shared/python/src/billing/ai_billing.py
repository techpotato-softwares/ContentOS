...
def resolve_ai_credentials(tenant: Any, *, preferred_provider: str | None = None) -> ResolvedAiCredentials:
    """Platform → AI_SECRET_ID/env; BYOK → tenant-stored keys; stub → empty keys."""
...
def meter_ai_success(
    ...
    *,
    ...
    """
    After successful AI:
    - platform → assert_and_consume_quota + usage event
    - byok → usage event only (no platform quota)
    - stub → optional usage event outside production
    """
    mode = (getattr(tenant, "ai_billing_mode", None) or DEFAULT_AI_BILLING_MODE).strip().lower()
