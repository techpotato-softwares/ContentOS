...
class Tenant(SQLModel, table=True):
    ...
    tenant_id: int | None = Field(default=None, primary_key=True)
    name: str
    ...
    context_pack_cached: str | None = Field(default=None, sa_column=Column(Text))
...
class User(SQLModel, table=True):
    ...
    tenant_id: int | None = Field(default=None, foreign_key="tenants.tenant_id", index=True)
...
class AuthToken(SQLModel, table=True):
    ...
    created_at: datetime = Field(default_factory=_utcnow)
    request_key: str | None = Field(default=None, index=True)  # hashed email for rate limits
...
class GenerationBatch(SQLModel, table=True):
    ...
    session_id: int | None = Field(default=None, foreign_key="chat_sessions.session_id")
...
class ContentPost(SQLModel, table=True):
    ...
    tenant_id: int = Field(foreign_key="tenants.tenant_id", index=True)
    batch_id: int | None = Field(default=None, foreign_key="generation_batches.batch_id", index=True)
    user_id: int = Field(foreign_key="users.user_id")
    ...
    layout_json: str | None = Field(default=None, sa_column=Column(Text))
    score_json: str | None = Field(default=None, sa_column=Column(Text))
    source_type: str | None = None  # brief | url | pdf
    source_ref: str | None = Field(default=None, sa_column=Column(Text))
    ab_label: str | None = None  # A | B | C | hold
...
class SocialAccount(SQLModel, table=True):
    ...
    token_payload_encrypted: str | None = Field(default=None, sa_column=Column(Text))
...
class TenantInvite(SQLModel, table=True):
    ...
    invite_id: int | None = Field(default=None, primary_key=True)
    tenant_id: int = Field(foreign_key="tenants.tenant_id", index=True)
    ...
    expires_at: datetime
    invited_by: int | None = Field(default=None, foreign_key="users.user_id")
