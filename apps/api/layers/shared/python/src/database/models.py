...
class User(SQLModel, table=True):
    ...
    OAuth users may have ``password`` unset and ``google_sub`` / ``linkedin_sub`` set.
    LinkedIn *login* identity is ``linkedin_sub`` — never auto-creates SocialAccount.
    """
    ...
    # password | google | linkedin | password+google | password+linkedin | …
    auth_provider: str = Field(default="password", index=True)
    google_sub: Optional[str] = Field(default=None, unique=True, index=True)
    linkedin_sub: Optional[str] = Field(default=None, unique=True, index=True)
...
class OAuthLoginState(SQLModel, table=True):
    """CSRF state + OIDC nonce + one-time exchange codes for Google/LinkedIn login."""
