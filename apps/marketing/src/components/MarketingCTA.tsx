...
export function MarketingCTA({
  ...
  return (
    <section className="section border-t border-[var(--line)]">
      <div className="section-inner relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#14b8a0] via-[#0d7a6c] to-[#042f2a] px-6 py-14 text-white md:px-12 md:py-16">
        <div
        ...
        <p className="relative mt-4 max-w-xl text-lg text-white/85">{description}</p>
        <div className="relative mt-8 flex flex-wrap items-center gap-3">
          <Link
            href={primary.href}
            className="btn-demo btn-demo-light inline-flex items-center justify-center rounded-md bg-white px-5 py-3 text-sm font-semibold text-[#0a5c52] transition hover:bg-[#e8fff9] hover:text-[#042f2a]"
          >
            {primary.label}
          </Link>
          {secondary && (
            <Link
              href={secondary.href}
              className="inline-flex items-center justify-center rounded-md border border-white/35 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10"
            >
              {secondary.label}
            </Link>
          )}
        ...
        <div className="relative mt-7 grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div
              key={item}
              className="rounded-lg border border-white/15 bg-white/5 px-3 py-2.5 text-sm text-white/80"
            >
              {item}
            </div>
          ))}
