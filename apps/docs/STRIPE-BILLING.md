...
**Razorpay** is the INR/UPI gateway — see `RAZORPAY-BILLING.md`. A tenant may have only **one active gateway**; switching requires cancel-then-resubscribe.
...
1. Create four recurring Prices (USD monthly) matching `PLAN_CATALOG`:
   ...
   - Growth — $149
   - Scale — $399
   - Agency — $399 (or your chosen Agency price)
2. Copy each Price id (`price_…`) into env / Secrets Manager.
...
Use the CLI-printed `whsec_…` as `STRIPE_WEBHOOK_SECRET` while listening.
...
- Price id → `PLAN_CATALOG` tier → `tenant.plan_tier` + `ai_posts_quota_monthly`.
- `invoice.payment_failed` / past-due / canceled → soft-lock **platform** AI generate with `402 billing_required`.
...
Settings → **Billing** (`/settings/billing`): Upgrade (Checkout) and Manage billing (Portal).
