"""Hybrid AI billing ΓÇö plan catalog and safe public serializers (no API secrets)."""
...
class PlanLimits:
    ...
    def public_dict(self) -> dict[str, Any]:
        """API-safe plan row (INR/USD display hooks; no secrets)."""
        return {
            "id": self.id,
            "label": self.label,
            ...
            "display": {
                ...
                "inr": f"Γé╣{self.monthly_inr:,.0f}/mo",
            },
        }


# Approximate INR list prices for UI (product copy; not live FX).
# Source: FEATURE-HYBRID-AI-BILLING.md ┬º3
PLAN_CATALOG: dict[str, PlanLimits] = {
    ...
    "agency": PlanLimits(
        ...
        byok_allowed=True,
        byok_required=True,
        description="BYOK required ΓÇö platform AI COGS not included (0 platform quota).",
    ),
}
...
def apply_billing_defaults(tenant: Any, *, plan: str | None = None, mode: str | None = None) -> None:
    """Set starter/platform defaults on a Tenant instance (new or backfill)."""
    ...
    # Snapshot catalog quota onto the tenant (create / explicit defaulting)
...
def _is_secret_key(name: str) -> bool:
    ...
    if "customer_id" in compact:
        return False
    if compact.endswith("_secret") or compact.endswith("secret"):
        return True
    if "api_key" in compact or compact.endswith("apikey"):
        return True
    if "secret_arn" in compact:
        return True
    return False


def redact_secrets(value: Any) -> Any:
    """Recursively strip provider/API secrets from structures before serialization."""
...
def tenant_billing_public(tenant: Any) -> dict[str, Any]:
    """Safe billing slice for tenant API responses (never includes keys or secret ARNs)."""
